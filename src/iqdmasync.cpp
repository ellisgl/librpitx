...
void iqdmasync::SetIQSample(uint32_t Index, std::complex<float> sample, int Harmonic) {
    ...
    int IntAmplitude = (int) (mydsp.amplitude * 8.0) -
                       1; //Fixme 1e4 seems to work with SSB but should be an issue with classical IQ file
    ...
    sampletab[Index * registerbysample + 1] =
        (0x5A << 24) + (IntAmplitudePAD & 0x7) + (1 << 4) + (0 << 3); // Amplitude PAD
    ...
    if (IntAmplitude == -1) {
        ...
        sampletab[Index * registerbysample + 2] =
            (Originfsel & ~(7 << 12)) | (4 << 12); //Alternate is CLK : Fixme : do not work with clk2
    }
