...
gpio::~gpio() {
    if (gpioreg != NULL) unmapmem((void *) gpioreg, gpiolen);
}
...
uint32_t get_hwbase(void) {
    ...
    if ((fd = fopen(ranges_file, "rb")) == NULL) {
        ...
        if (!ret) ret = (ranges[8] << 24) | (ranges[9] << 16) | (ranges[10] << 8) | (ranges[11] << 0);
        if ((ranges[0] != 0x7e) || (ranges[1] != 0x00) || (ranges[2] != 0x00) || (ranges[3] != 0x00) ||
            ...
            dbg_printf(1, "Unexpected ranges data (%02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x)\n", ranges[0],
                       ranges[1], ranges[2], ranges[3], ranges[4], ranges[5], ranges[6], ranges[7], ranges[8],
                       ranges[9], ranges[10], ranges[11]);
            ret = 0;
        }
    } else {
...
uint32_t gpio::GetPeripheralBase() {
    ...
    dbg_printf(1, "Peri Base = %x SDRAM %x\n", /*get_hwbase()*/ bcm_host_get_peripheral_address(),
    ...
    if (pi_is_2711) dbg_printf(1, "Running on Pi4\n");
...
uint64_t clkgpio::GetPllFrequency(int PllNo) {
    ...
    switch (PllNo) {
        case clk_osc:
            ...
            break;
        case clk_plla:
            Freq = (XOSC_FREQUENCY * ((uint64_t) gpioreg[PLLA_CTRL] & 0x3ff) +
                    XOSC_FREQUENCY * (uint64_t) gpioreg[PLLA_FRAC] / (1 << 20)) /
        ...
        case clk_pllc:
            ...
            if ((gpioreg[PLLC_PER] != 0) && (gpioreg[PLLC_CTRL] >> 12 != 0))
                Freq = ((XOSC_FREQUENCY * ((uint64_t) gpioreg[PLLC_CTRL] & 0x3ff) +
                         (XOSC_FREQUENCY * (uint64_t) gpioreg[PLLC_FRAC]) / (1 << 20)) /
                        (2 * gpioreg[PLLC_PER] >> 1)) /
                       ((gpioreg[PLLC_CTRL] >> 12) & 0x7);
            break;
        case clk_plld:
            Freq = ((XOSC_FREQUENCY * ((uint64_t) gpioreg[PLLD_CTRL] & 0x3ff) +
                     (XOSC_FREQUENCY * (uint64_t) gpioreg[PLLD_FRAC]) / (1 << 20)) /
                    (2 * gpioreg[PLLD_PER] >> 1)) /
                   ((gpioreg[PLLD_CTRL] >> 12) & 0x7);
            break;
        case clk_hdmi:
            Freq = (XOSC_FREQUENCY * ((uint64_t) gpioreg[PLLH_CTRL] & 0x3ff) +
                    XOSC_FREQUENCY * (uint64_t) gpioreg[PLLH_FRAC] / (1 << 20)) /
...
int clkgpio::SetFrequency(double Frequency) {
    if (ModulateFromMasterPLL) {
        ...
        if (PllFixDivider == 1) //Using PDIV thus frequency/2
        {
            if (pi_is_2711)
                FloatMult =
                    ((double) (CentralFrequency + Frequency) * 2) / ((double) (XOSC_FREQUENCY) * (1 - clk_ppm * 1e-6));
            else {
                FloatMult =
                    ((double) (CentralFrequency + Frequency)) / ((double) (XOSC_FREQUENCY * 2) * (1 - clk_ppm * 1e-6));
            }

        } else {
            if (pi_is_2711)
                FloatMult = ((double) (CentralFrequency + Frequency) * PllFixDivider * 2) /
                            ((double) (XOSC_FREQUENCY) * (1 - clk_ppm * 1e-6));
            else {
                FloatMult = ((double) (CentralFrequency + Frequency) * PllFixDivider) /
                            ((double) (XOSC_FREQUENCY) * (1 - clk_ppm * 1e-6));
            }
        }
        ...
        if ((FreqDivider > 4096) || (FreqDivider < 2)) dbg_printf(0, "Frequency out of range\n");
...
#define XTAL_RATE XOSC_FREQUENCY
    ...
    if (Frequency * 2 > MIN_PLL_RATE_USE_PDIV) {
        ...
        {
            if (Frequency * divider < MIN_PLL_RATE) continue; // widest accepted frequency range
    ...
    if (best_divider != 0) {
        ...
        dbg_printf(1, "Found PLL solution for frequency %4.1fMHz : divider:%d VCO: %4.1fMHz\n", (Frequency / 1e6),
                   PllFixDivider, (Frequency / 1e6) * ((PllFixDivider == 1) ? 2.0 : (double) PllFixDivider));
        return 0;
    } else {
...
double clkgpio::GetRealFrequency(double Frequency) {
    ...
    double RealFrequency =
        ((double) IntMultiply + (FracMultiply / (double) (1 << 20))) * (double) (XOSC_FREQUENCY) / PllFixDivider -
...
void clkgpio::print_clock_tree(void) {
    ...
    dbg_printf(1, "PLLA PDIV=%d NDIV=%d FRAC=%d  ", (gpioreg[PLLA_CTRL] >> 12) & 0x7, gpioreg[PLLA_CTRL] & 0x3ff,
               gpioreg[PLLA_FRAC]);
    dbg_printf(1, " %f MHz\n",
               xoscmhz * ((float) (gpioreg[PLLA_CTRL] & 0x3ff) + ((float) gpioreg[PLLA_FRAC]) / ((float) (1 << 20))));
    dbg_printf(1, "DSI0=%d CORE=%d PER=%d CCP2=%d\n\n", gpioreg[PLLA_DSI0], gpioreg[PLLA_CORE], gpioreg[PLLA_PER],
               gpioreg[PLLA_CCP2]);

    dbg_printf(1, "PLLB PDIV=%d NDIV=%d FRAC=%d  ", (gpioreg[PLLB_CTRL] >> 12) & 0x7, gpioreg[PLLB_CTRL] & 0x3ff,
               gpioreg[PLLB_FRAC]);
    dbg_printf(1, " %f MHz\n",
               xoscmhz * ((float) (gpioreg[PLLB_CTRL] & 0x3ff) + ((float) gpioreg[PLLB_FRAC]) / ((float) (1 << 20))));
    dbg_printf(1, "ARM=%d SP0=%d SP1=%d SP2=%d\n\n", gpioreg[PLLB_ARM], gpioreg[PLLB_SP0], gpioreg[PLLB_SP1],
               gpioreg[PLLB_SP2]);

    dbg_printf(1, "PLLC PDIV=%d NDIV=%d FRAC=%d  ", (gpioreg[PLLC_CTRL] >> 12) & 0x7, gpioreg[PLLC_CTRL] & 0x3ff,
               gpioreg[PLLC_FRAC]);
    dbg_printf(1, " %f MHz\n",
               xoscmhz * ((float) (gpioreg[PLLC_CTRL] & 0x3ff) + ((float) gpioreg[PLLC_FRAC]) / ((float) (1 << 20))));
    dbg_printf(1, "CORE2=%d CORE1=%d PER=%d CORE0=%d\n\n", gpioreg[PLLC_CORE2], gpioreg[PLLC_CORE1], gpioreg[PLLC_PER],
               gpioreg[PLLC_CORE0]);

    dbg_printf(1, "PLLD %x PDIV=%d NDIV=%d FRAC=%d  ", gpioreg[PLLD_CTRL], (gpioreg[PLLD_CTRL] >> 12) & 0x7,
               gpioreg[PLLD_CTRL] & 0x3ff, gpioreg[PLLD_FRAC]);
    dbg_printf(1, " %f MHz\n",
               xoscmhz * ((float) (gpioreg[PLLD_CTRL] & 0x3ff) + ((float) gpioreg[PLLD_FRAC]) / ((float) (1 << 20))));
    dbg_printf(1, "DSI0=%d CORE=%d PER=%d DSI1=%d\n\n", gpioreg[PLLD_DSI0], gpioreg[PLLD_CORE], gpioreg[PLLD_PER],
               gpioreg[PLLD_DSI1]);

    dbg_printf(1, "PLLH PDIV=%d NDIV=%d FRAC=%d  ", (gpioreg[PLLH_CTRL] >> 12) & 0x7, gpioreg[PLLH_CTRL] & 0x3ff,
               gpioreg[PLLH_FRAC]);
    dbg_printf(1, " %f MHz\n",
               xoscmhz * ((float) (gpioreg[PLLH_CTRL] & 0x3ff) + ((float) gpioreg[PLLH_FRAC]) / ((float) (1 << 20))));
    dbg_printf(1, "AUX=%d RCAL=%d PIX=%d STS=%d\n\n", gpioreg[PLLH_AUX], gpioreg[PLLH_RCAL], gpioreg[PLLH_PIX],
               gpioreg[PLLH_STS]);
}
...
void clkgpio::SetppmFromNTP() {
    ...
    if (status != TIME_OK) {
        ...
        if (fabs(ntp_ppm) < 200) Setppm(ntp_ppm /*+0.70*/); //0.7 is empiric
    }
}
...
int pwmgpio::SetFrequency(uint64_t Frequency) {
    ...
    uint32_t FreqDivider = (uint32_t) Freqresult;
    uint32_t FreqFractionnal = (uint32_t) (4096 * (Freqresult - (double) FreqDivider));
    if ((FreqDivider > 4096) || (FreqDivider < 2)) dbg_printf(1, "Frequency out of range\n");
...
void pwmgpio::SetMode(int Mode) {
    if ((Mode >= pwm1pin) && (Mode <= pwm1pinrepeat)) ModePwm = Mode;
}
...
int pcmgpio::SetFrequency(uint64_t Frequency) {
    ...
    if ((FreqDivider > 4096) || (FreqDivider < 2)) dbg_printf(1, "PCM Frequency out of range\n");
    clk.gpioreg[PCMCLK_DIV] = 0x5A000000 | ((FreqDivider) << 12) | FreqFractionnal;
    SetPrediv(Prediv);
    return 0;
}

int pcmgpio::SetPrediv(
    int predivisor) //Carefull we use a 10 fixe divisor for now : frequency is thus f/10 as a samplerate
{
    if (predivisor > 1000) {
