...
ookburst::ookburst(uint64_t TuneFrequency, float SymbolRate, int Channel, uint32_t FifoSize, size_t upsample,
                   float RatioRamp)
...
void ookburst::SetSymbols(unsigned char *Symbols, uint32_t Size) {
    ...
    for (unsigned i = 0; i < Size; i++) {
        ...
        for (size_t j = 0; j < Ramp; j++) {
            if (i < Size - 1) {
                sampletab[i * SR_upsample + j + SR_upsample - Ramp] = (Symbols[i] == 0)
                                                                          ? ((Originfsel & ~(7 << 12)) | (0 << 12))
                                                                          : ((Originfsel & ~(7 << 12)) | (4 << 12));
...
ookbursttiming::~ookbursttiming() {
    if (ookrenderbuffer != nullptr) delete[] ookrenderbuffer;
}
